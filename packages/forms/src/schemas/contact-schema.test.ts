 ...
 * DEPENDS ON: vitest and the contactFormSchema exported from ./contact-schema.
...
describe('contactFormSchema', () => {
  ...
  it.each(['website', 'traffic-leads', 'other'] as const)(
    ...
    (projectType) => {
      const result = contactFormSchema.safeParse({ ...validInput, projectType });
      expect(result.success).toBe(true);
    }
  );
  ...
  it('fails when name is missing', () => {
    ...
    const result = contactFormSchema.safeParse(inputWithoutName);
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  it('fails when email is missing', () => {
    ...
    const result = contactFormSchema.safeParse(inputWithoutEmail);
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  it('fails when message is missing', () => {
    ...
    const result = contactFormSchema.safeParse(inputWithoutMessage);
    expect(result.success).toBe(false);
    if (!result.success) {
